...
if ($deleta == "s"){
	for ($i = 1; $i <= 1000; $i++){
		if (isset($_POST['chk' . $i])){
			...
			$qr  = "DELETE FROM prod_material WHERE prod_material.PRM_MATERIAL = '$del' ";
			mysql_query($qr);
		}
	}
...
/* ##################### CONSULTA #####################  */
$qr = "SELECT * FROM prod_material ORDER BY prod_material.PRM_PRIORID ";
$sql = mysql_query($qr);
...
<form id="formCadMaterial" name="formCadMaterial" action="del_material.php?deleta=s" method="post" accept-charset="utf-8">
				...
				<table border="1" cellspacing="0" cellpadding="0" >
					<thead>
						<tr class="ui-widget-header" height="30px">
							...
							<td align="left"><font size="2"><b>Material</b></font></td>
					...
					<?php
						...
						while($r = mysql_fetch_array($sql)){
							echo "<tr>".
								 "<td align='center'><input type='checkbox' name='chk". $indice ."' value='". $r['PRM_MATERIAL'] ."'></td>" .
								 "<td align='left'>"   . $r['PRM_MATERIAL'] . "&nbsp;&nbsp;</td>" . 
								 "<td align='left'>"   . $r['PRM_MATERIAL_DESC'] . "&nbsp;&nbsp;</td>" . 
								 "<td align='center'>" . $r['PRM_PRIORID'] . "</td>".
								 "</tr>";
							$indice = $indice + 1;
						}
					?>

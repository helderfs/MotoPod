...
if (isset($_POST['MOT_OLEO']))		echo "<br>ini1". $_POST['MOT_OLEO'];
	// CHAVES PRIMARIAS
	...
	if (isset($_POST['hdMMO_MODELO'])){
		$mod_cod 				   = retSEspOut( $_POST['hdMMO_MODELO'] );
		$_SESSION['sesMMO_MODELO'] = $mod_cod;
	}
	...
	if (isset($_POST['MOT_TRANSPRI']))	$MOT_TRANSPRI	= $_POST['MOT_TRANSPRI'];
	if (isset($_POST['MOT_TRANSSEC']))	$MOT_TRANSSEC	= $_POST['MOT_TRANSSEC'];
	if (isset($_POST['MOT_TANQUE']))	$MOT_TANQUE	 	= $_POST['MOT_TANQUE'];
	if (isset($_POST['MOT_DIMENS']))	$MOT_DIMENS	 	= $_POST['MOT_DIMENS'];
	...
	if (isset($_POST['MOT_IGNI']))		$MOT_IGNI		= $_POST['MOT_IGNI'];
	...
	if (isset($_POST['MOT_CAPCARGA']))	$MOT_CAPCARGA	= $_POST['MOT_CAPCARGA'];
	...
	if ($MOT_CATEG == ""){
		$c_erros .= ",Categoria não informado.";
		$er_MOT_CATEG = ".";
	}
	if ($MOT_CC == ""){
		$c_erros .= ",Cilindrada não informado.";
		$er_MOT_CC = ".";
	}
	if ($MOT_CIL == ""){
...
echo "<br>ENTROU COPIA ???". $MOT_OLEO;
	...
	$_SESSION['sesMMO_MARCA'] 	= "";
	$_SESSION['sesMMO_MODELO'] 	= "";
echo "<br>ENTROU COPIA 2222 ???". $MOT_OLEO;
}

echo "<br>PASSOU COPIA ???". $MOT_OLEO;

/* ##################### INSERCAO ##################### */
if ($btIncluir != ""){
echo "<br>ENTROU INSERCAO ???". $MOT_OLEO;
	iniFields();
		...
		for($i = 0 ; $i < 6 ; $i++ ){
			if( $_FILES ) { // Verificando se existe o envio de arquivos.
				if( $_FILES['MIM_PATH'.$i]['name'] <> "" ) { // Verifica se o campo não está vazio.
					...
					if(!is_dir("../imgMotoPod/Moto/". $marca_cod))
						 mkdir("../imgMotoPod/Moto/". $marca_cod);
					if(!is_dir("../imgMotoPod/Moto/". $marca_cod ."/". $mod_cod ."/"))
						 mkdir("../imgMotoPod/Moto/". $marca_cod ."/". $mod_cod ."/");
					if(!is_dir("../imgMotoPod/Moto/". $marca_cod ."/". $mod_cod ."/". $MOT_ANOFAB ."/"))
						 mkdir("../imgMotoPod/Moto/". $marca_cod ."/". $mod_cod ."/". $MOT_ANOFAB ."/");
					// NAO ADIANTA FAZER DESTA SELECAO UMA FUNCAO

					$dir 	 	= "../imgMotoPod/Moto/";
					$dir_cat_cd	= $marca_cod ."/". $mod_cod ."/". $MOT_ANOFAB ."/";
	...
	if ($c_erros != ""){
		...
		$num_busca = mysql_num_rows($exe_busca);
		if ($num_busca == 0){
			$sql_busca = "INSERT INTO mp_motos (MOT_MMO_COD, MOT_ANOFAB, MOT_CATEG, MOT_CC, MOT_CIL, MOT_TPMOTOR, MOT_HP, MOT_KGFM, MOT_MARCHAS, MOT_LUB, MOT_OLEO, MOT_DIA_CUR, MOT_TXCOMPR, MOT_ALIMENT, MOT_IGNI, MOT_EMBREAG, MOT_COMB, MOT_PARTIDA, MOT_TRANSPRI, MOT_TRANSSEC, MOT_TANQUE, MOT_TPCHASSI, MOT_DIMENS, MOT_EIXODIS, MOT_PNEUD, MOT_PNEUT, MOT_LAMPDIA, MOT_LAMPTRA, MOT_LAMPEJ, MOT_PISCAAL, MOT_SISELE, MOT_BATERIA, MOT_PESO, MOT_SUSPDIA, MOT_SUSPTRA, MOT_COMPRIM, MOT_LARG, MOT_ALTURA, MOT_ALTASS, MOT_DISTSOLO, MOT_CAPCARGA, MOT_FREIOD, MOT_FREIOT)
...
echo "<br>PASSOU INSERCAO ???". $MOT_OLEO;
/* ##################### CONSULTA ##################### */
if ( ($MOT_MMO_COD != "" || ($SelEdtMtCod != "" && $SelEdtMtAno != "")) && ($c_erros == "") ){
echo "<br>ENTROU CONSULTA ???". $MOT_OLEO;
	...
	if ($num_busca <> 0){
		...
		$MOT_CAPCARGA   = $fet_busca['MOT_CAPCARGA'];
...
echo "<br>ENTROU DELETA TUDO ???". $MOT_OLEO;
	...
	if ($num_busca <> 0){
		...
		$mod_cod   = retSEspOut( $fet_busca['MMO_MODELO'] );
	}

	// Apaga Diretorio de Ano do Modelo e seus arquivos 
	if( is_dir("../imgMotoPod/Moto/". $marca_cod ."/". $mod_cod ."/". $SelDelMtAno ."/") )
		rmdir_recurse("../imgMotoPod/Moto/". $marca_cod ."/". $mod_cod ."/". $SelDelMtAno);

	zeraCampos();
}
echo "<br>PASSOU DELETA TUDO ???". $MOT_OLEO;

/* ##################### DELETA TODAS IMAGENS do BANCO DADOS e ARQUIVOS ##################### */
if ($SelDelMtImgCod != "" && $SelDelMtImgAno != ""){
echo "<br>ENTROU DELETA IMAGENS ???". $MOT_OLEO;
	if ($SelDelMtImgPath != ""){
		$qr = "DELETE FROM mp_moto_img 
		...
		// Deleta imagem
		unlink("../imgMotoPod/Moto/". $marca_cod ."/". $mod_cod ."/". $MOT_ANOFAB ."/". $SelDelMtImgPath);
	}else{
		$qr = "DELETE FROM mp_moto_img 
					 WHERE MIM_MMO_COD = '$SelDelMtImgCod'
					   AND MIM_MOT_ANO = '$MOT_ANOFAB' ";
		if( is_dir("../imgMotoPod/Moto/". $marca_cod ."/". $mod_cod ."/". $MOT_ANOFAB) )
			rmdir_recurse("../imgMotoPod/Moto/". $marca_cod ."/". $mod_cod ."/". $MOT_ANOFAB);
	}
...
echo "<br>PASSOU DELETA IMAGENS ???". $MOT_OLEO;
...
echo "<br>PASSOU FUNCOES ???". $MOT_OLEO;
			...
			<div id="tabs-1">
				<table border="0" cellspacing="0" cellpadding="0" width="610px" align="center">
							...
							<table border="0" cellspacing="0" cellpadding="0" align="center" width="100%" height="25px">
								...
								<tr align="center">
									<td>
										...
										&nbsp;&nbsp;
										<a href="javascript:janelaSecundaria('mod/del_moto_marca.php', 700, 500);" title="Exkluir"><img src="images/icons/minus.png"></a>
									</td>
									<td>
										<a href="javascript:janelaSecundaria('mod/cadmotomod.php', 700, 500);" title="Incluir"><img src="images/icons/plus.png"></a>
										&nbsp;&nbsp;
										<a href="javascript:janelaSecundaria('mod/del_moto_mod.php', 700, 500);" title="Exkluir"><img src="images/icons/minus.png"></a>
									</td>
								</tr>
							</table></br>
						</td>
					...
					<tr>
						<td colspan="2" valign="middle">
							<iframe name="iframeMarMod" id="iframeMarMod" src="./ajax_marca_mod.php" frameborder="0" scrolling="no" align="center" width="85%" height="62px">
					...
					<tr>
						...
						<td align="left">
							<select name="MOT_CATEG" class="<?php if($er_MOT_CATEG != "") echo "field_error"; ?>">
								...
								<option value="C" <?php if ($MOT_CATEG == "L") echo "selected"; else echo ""; ?>>Clássica</option>
				...
				<tr>
					...
					<tr>
						...
						<td align="left"><input type="text" class="<?php if($er_MOT_CIL != "")echo "field_error"; else echo "input"; ?>" size="2" maxlength="2" name="MOT_CIL" value="<?php if($MOT_CIL != ""){ echo $MOT_CIL; } ?>"></td>
					</tr>
					<tr>
						<td class="td_cad" align="right"><div align="right"><font color="red">* </font><b><label style="margin-right:5px;">Cilindrada</label></b></td>
						<td align="left"><input type="text" class="<?php if($er_MOT_CC != "") echo "field_error"; ?>" size="6" maxlength="6" name="MOT_CC" value="<?php if($MOT_CC != ""){ echo $MOT_CC; } ?>"></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td align="left"><input type="text" class="<?php if($er_MOT_TPMOTOR != "")echo "field_error"; else echo "input"; ?>" size="40" maxlength="90" name="MOT_TPMOTOR" value="<?php if($MOT_TPMOTOR != ""){ echo $MOT_TPMOTOR; } ?>"></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td align="left"><input type="text" size="40" maxlength="40" name="MOT_OLEO" value="<?php if($MOT_OLEO != "") echo $MOT_OLEO; ?>"></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td align="left"><input type="text" size="40" maxlength="90" name="MOT_ALIMENT" value="<?php if($MOT_ALIMENT != ""){ echo $MOT_ALIMENT; } ?>"></td>
					</tr>
					<tr>
					...
					<tr>
						<td class="td_cad" align="right"><div align="right"><b><label style="margin-right:5px;">Transmissão Primária</label></b></td>
						<td align="left">
							<select name="MOT_TRANSPRI">
								<option value="E" <?php if ($MOT_TRANSPRI == "E" || $MOT_TRANSPRI == "") echo "selected"; else echo ""; ?>>Engrenagens</option>
								<option value="C" <?php if ($MOT_TRANSPRI == "C") echo "selected"; else echo ""; ?>>Corrente</option>
							</select>
						</td>
					</tr>
					<tr>
						...
						<td align="left">
							<select name="MOT_TRANSSEC">
								...
								<option value="O" <?php if ($MOT_TRANSSEC == "O") echo "selected"; else echo ""; ?>>Correia</option>
							</select>
						</td>
					</tr>
				...
				<tr>
					...
					<tr>
						...
						<td align="left"><input type="text" size="40" maxlength="90" name="MOT_TPCHASSI" value="<?php if($MOT_TPCHASSI != ""){ echo $MOT_TPCHASSI; } ?>"></td>
					</tr>
					<tr>
						<td class="td_cad" align="right"><div align="right"><b><label style="margin-right:5px;">Suspensão Dianteira</label></b></td>
						<td align="left"><input type="text" size="40" maxlength="90" name="MOT_SUSPDIA" value="<?php if($MOT_SUSPDIA != ""){ echo $MOT_SUSPDIA; } ?>"></td>
					</tr>
					<tr>
						<td class="td_cad" align="right"><div align="right"><b><label style="margin-right:5px;">Suspensão Traseira</label></b></td>
						<td align="left"><input type="text" size="40" maxlength="90" name="MOT_SUSPTRA" value="<?php if($MOT_SUSPTRA != ""){ echo $MOT_SUSPTRA; } ?>"></td>
					</tr>
				...
				<tr>
					...
					<tr>
						...
						<td align="left"><input type="text" class="<?php if($er_MOT_EIXODIS != "")echo "field_error"; else echo "input"; ?>" size="10" maxlength="10" name="MOT_EIXODIS" value="<?php if($MOT_EIXODIS != ""){ echo $MOT_EIXODIS; } ?>"></td>
					</tr>
					<tr>
						<td class="td_cad" align="right"><div align="right"><b><label style="margin-right:5px;">Comprimento</label></b></td>
						<td align="left"><input type="text" size="10" maxlength="9" name="MOT_COMPRIM" value="<?php if($MOT_COMPRIM != "") echo $MOT_COMPRIM; ?>"></td>
					</tr>
					<tr>
						<td class="td_cad" align="right"><div align="right"><b><label style="margin-right:5px;">Largura</label></b></td>
						<td align="left"><input type="text" size="10" maxlength="9" name="MOT_LARG" value="<?php if($MOT_LARG != "") echo $MOT_LARG; ?>"></td>
					</tr>
					<tr>
						<td class="td_cad" align="right"><div align="right"><b><label style="margin-right:5px;">Altura</label></b></td>
						<td align="left"><input type="text" size="10" maxlength="9" name="MOT_ALTURA" value="<?php if($MOT_ALTURA != "") echo $MOT_ALTURA; ?>"></td>
					</tr>
					<tr>
						<td class="td_cad" align="right"><div align="right"><b><label style="margin-right:5px;">Altura do Assento</label></b></td>
						<td align="left"><input type="text" size="10" maxlength="9" name="MOT_ALTASS" value="<?php if($MOT_ALTASS != "") echo $MOT_ALTASS; ?>"></td>
					</tr>
					<tr>
						<td class="td_cad" align="right"><div align="right"><b><label style="margin-right:5px;">Distância Solo</label></b></td>
						<td align="left"><input type="text" size="10" maxlength="9" name="MOT_DISTSOLO" value="<?php if($MOT_DISTSOLO != "") echo $MOT_DISTSOLO; ?>"></td>
					</tr>
					<tr>
					...
					<tr>
						<td class="td_cad" align="right"><div align="right"><b><label style="margin-right:5px;">Capacidade de Carga</label></b></td>
						<td align="left"><input type="text" size="40" maxlength="40" name="MOT_CAPCARGA" value="<?php if($MOT_CAPCARGA != "") echo $MOT_CAPCARGA; ?>"></td>
					</tr>
					<tr>
				...
				<tr>
					...
					<tr>
						...
						<td align="left"><input type="text" size="40" maxlength="90" name="MOT_FREIOD" value="<?php if($MOT_FREIOD != "") echo $MOT_FREIOD; ?>"></td>
					</tr>
					<tr>
						<td class="td_cad" align="right"><div align="right"><b><label style="margin-right:5px;">Freio Traseiro</label></b></td>
						<td align="left"><input type="text" size="40" maxlength="90" name="MOT_FREIOT" value="<?php if($MOT_FREIOT != "") echo $MOT_FREIOT; ?>"></td>
					</tr>
					<tr>
						<td class="td_cad" align="right"><div align="right"><font color="red">* </font><b><label style="margin-right:5px;">Pneu Dianteiro</label></b></td>
						<td align="left"><input type="text" class="<?php if($er_MOT_PNEUD != "")echo "field_error"; else echo "input"; ?>" size="40" maxlength="90" name="MOT_PNEUD" value="<?php if($MOT_PNEUD != "") echo $MOT_PNEUD; ?>"></td>
					</tr>
					<tr>
						<td class="td_cad" align="right"><div align="right"><font color="red">* </font><b><label style="margin-right:5px;">Pneu Traseiro</label></b></td>
						<td align="left"><input type="text" class="<?php if($er_MOT_PNEUT != "")echo "field_error"; else echo "input"; ?>" size="40" maxlength="90" name="MOT_PNEUT" value="<?php if($MOT_PNEUT != "") echo $MOT_PNEUT; ?>"></td>
					</tr>
				...
				if ($MOT_MMO_COD != ""){
					...
					while($r = mysql_fetch_array($sql)){
						...
						<tr>
							...
							<td align="left">
								<img src='<?php echo "../imgMotoPod/Moto/". $marca_cod ."/". $mod_cod ."/". $MOT_ANOFAB ."/". $path_img; ?>' width='80px' height='50px'>
							</td>
